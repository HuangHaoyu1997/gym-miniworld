...
class FourRooms(MiniWorldEnv):
    ...
    def _gen_world(self):
        ...
        # 放置多个目标
        self.box_red = self.place_entity(Box(color='red',size=1.5),min_x=-4,max_x=-4,min_z=4,max_z=4)
        self.key_blue = self.place_entity(Ball(color='blue'),min_x=4,max_x=4,min_z=4,max_z=4)
        self.ball_green = self.place_entity(Ball(color='green'),min_x=-4,max_x=-4,min_z=-4,max_z=-4)
        self.box_purple = self.place_entity(Box(color='purple'),min_x=4,max_x=4,min_z=-4,max_z=-4)
        '''
        ...
        '''
        self.img_wall = self.place_entity(TextFrame(pos=[-5,1.5,5],dir=0,str='PAVES'),room=0)
